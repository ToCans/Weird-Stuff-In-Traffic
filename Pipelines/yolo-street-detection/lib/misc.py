...
def train_model(pretrained_model='models/yolo11n-seg.pt', save_name="default_name", imgsz=64, epochs=200, batch=64, amp=True, workspace="someworkspace", usecomet=True):
    if usecomet:
        experiment = Experiment(api_key="GMZvaRkubQuwhKdidrCBz3cMv", project_name="streetseg", workspace=workspace)

    # clear folder first
    os.makedirs(f"yolov11finetune_street/{save_name}", exist_ok=True)
    
    for folder in [f"yolov11finetune_street/{save_name}"]:
    ...
    try:
        ...
        results = model.train(
            model=model,
            data='data_seg/yolo_seg_dataset.yaml',
            save=True,
            patience=0,
            val=True,
            device=0,
            ...
            project='yolov11finetune_street', # Same as your comet project name
            amp=amp,
    ...
    shutil.copyfile(f"yolov11finetune_street/{save_name}/weights/last.pt", f"models/streetseg_{imgsz}_auto.pt")
